...
public class Demo {
	...
	public static Match parseMatch(String s, int cursor) {
		...
		String sWhen = s.substring(s.indexOf("match-time\">", cursor) + 12, s.indexOf("<", s.indexOf("match-time\">", cursor) + 12));
		...
		String where = s.substring(s.indexOf("eventm\">", cursor) + 8, s.indexOf("<", s.indexOf("eventm\">", cursor) + 8));
		cursor = s.indexOf("<", s.indexOf("eventm\">", cursor) + 8);
		
		long id = Long.parseLong(s.substring(s.indexOf("match?m=", cursor) + 8, s.indexOf("\">", s.indexOf("match?m=", cursor) + 8)));
		...
		String team1 = s.substring(s.indexOf("b>", cursor) + 2, s.indexOf("<", s.indexOf("b>", cursor) + 2));
		cursor = s.indexOf("<", s.indexOf("b>", cursor) + 2);
		
		cursor = s.indexOf("teamtext\">", cursor) + 10;
		int team2cursor = cursor;
		String team2 = s.substring(s.indexOf("b>", cursor) + 2, s.indexOf("<", s.indexOf("b>", cursor) + 2));
		cursor = s.indexOf("<", s.indexOf("b>", cursor) + 2);
		
		if ((resultCursor > matchCursor)&&(resultCursor < team1cursor)) {
			result = 1;
		}
		else if ((resultCursor > matchCursor)&&(resultCursor > team1cursor)&&(resultCursor < team2cursor)){
			result = 2;
			}
